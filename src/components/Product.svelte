	...
	let productFeatures = [
		...
		},
		{
			...
			featureList: [
				...
				"Guaarantees progress on all core exercises"
			],
	...
	>
		<div class="flex flex-col gap-2">
			<p class="opacity-60 text-base sm:text-lg md:text-xl text-center">
				Start your lifeline <span class="text-indigo-400">gym streak</span>
				today
			</p>
			<h3
		...
		{#each productFeatures as productFeature, index}
			<ProductCard {productFeature} {index}>
				{#if index === 0}
					...
					> class
				{:else if index === 1}
					Lay the foundation for a <span class="text-indigo-400 font-medium"
						>routine</span
					> and consistency
				{:else}
