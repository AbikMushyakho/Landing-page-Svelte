...
>
	...
	>
		<h4
			class="font-semibold text-2xl sm:text-3xl md:text-4xl max-w-[1000px] w-full relative pr-10 after:absolute after:top-full after:left-0 after:w-1/5 after:h-1.5 after:mt-1 after:bg-slate-900"
		>
			<slot />
		</h4>
		<p>{productFeature.description}</p>
		<div class="flex flex-col gap-3">
			{#each productFeature.featureList as listItem (listItem)}
	...
	>
		...
		<div class="flex flex-col bg-white gap-4 flex-1">
			<img src={productFeature.imgUrl} alt="productImage" />
		</div>
	</div>
</div>
